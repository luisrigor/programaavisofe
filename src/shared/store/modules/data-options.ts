...
import AuthenticationInitialApi from 'pages/authenticationInitial/AuthenticationInitialApi';
...
export const useDataOptions = defineStore('dataOptions', () => {
  ...
  const generateDataSelect = async () => {
    ...
    while (tempNum < 12) {
      ...
      dataOptionsStore.dataSelect.months[tempNum] = date.toLocaleString(
        'pt-PT',
        { month: 'long' }
      );
      tempNum++;
